...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Toggle para a seção "Separadores Inativos"
  const toggleInactiveBtn = document.getElementById('toggleInactiveSeparators');
  toggleInactiveBtn.addEventListener('click', function() {
    const table = document.getElementById('inactiveSeparators');
    if (table.style.display === 'none' || table.style.display === '') {
      table.style.display = 'table';
      this.textContent = 'Ocultar Seção';
    } else {
...
// Processa os separadores inativos persistindo-os
...
// Atualiza a tabela de Separadores Inativos
function updateInactiveSeparatorsTable(inactiveSeparators) {
  ...
  const tbody = document.getElementById('inactiveSeparatorsBody');
  if (inactiveSeparators.length === 0) {
    tbody.innerHTML = `<tr><td colspan="5">Nenhum separador inativo no momento</td></tr>`;
    document.getElementById('inactiveSeparatorCount').textContent = '(Total: 0)';
    return;
  }
  
  tbody.innerHTML = inactiveSeparators.map(item => {
  ...
  document.getElementById('inactiveSeparatorCount').textContent = `(Total: ${inactiveSeparators.length})`;
}
