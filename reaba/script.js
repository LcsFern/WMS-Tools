...
let ondas = JSON.parse(localStorage.getItem('ondasReabastecimentos)) || [];
...
function processarMovimentacao(rawText) {
  ...
  localStorage.setItem('ondasReabastecimentoseabastecimentos, JSON.stringify(ondas));
  ...
  [...pendentes, ...finalizadas].forEach(onda => {
    ...
    header.innerHTML = `
      ...
      <div class="group-actions">
        <button class="btnMostrar">Mostrar Resbastecimentos</button>
    ...
    btnMostrar.addEventListener('click', () => {
      if (details.style.display === 'none' || !details.style.display) {
        ...
        btnMostrar.textContent = 'Ocultar Reabastecimentos';
      } else {
        details.style.display = 'none';
        btnMostrar.textContent = 'Mostrar Reabastecimentos';
      }
    });
    ...
    if (btnExportarCongelado) {
      btnExportarCongelado.addEventListener('click', (e) => {
        ...
        localStorage.setItem('ondasReabastecimentos, JSON.stringify(ondas));
...
function exportarPDF(ondaNum, tipo, registros) {
  ...
  // Adiciona dados ou linhas vazias (garantindo que tem pelo menos 8 linhas)
  const totalLinhas = Math.max(8, registros.length);
  ...
  doc.save(`Picking_Reabastecimento_ONDA_${ondaNum}_${tipo}.pdf`);
}
